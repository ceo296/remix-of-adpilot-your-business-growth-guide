...
4. כשהקמפיין קשור לחג — הנח את סוכן הקריאייטיב לשלב ניואנסים מגזריים ספציפיים.
   אבל כשאין חג ספציפי — חל איסור מוחלט על סמלים דתיים/פולחניים (חנוכייה, גביע, שופר, קערת סדר, לולב, מגילה, נרות שבת). הוויזואל חייב להתמקד אך ורק במוצר/שירות. שילוב סמל דתי במודעה שלא קשורה לחג = טעות מקצועית חמורה.
   כשהקמפיין קשור לחג:
  ...
  try {
    ...
    if (!aiSuccess && LOVABLE_API_KEY) {
      ...
      const aiResponse = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
        ...
        body: JSON.stringify({ model: 'google/gemini-2.5-pro', messages: textMessages, max_completion_tokens: 8192 }),
      });
