...
const vibeLabels: Record<CreativeVibe, { label: string; tag: string; icon: React.ReactNode; color: string }> = {
  aggressive: { label: 'צועק ומכירתי', tag: 'STYLE_AGGRESSIVE', icon: <Zap className="w-5 h-5" />, color: 'bg-red-500' },
  prestige: { label: 'יוקרתי ונקי', tag: 'STYLE_PRESTIGE', icon: <Crown className="w-5 h-5" />, color: 'bg-amber-500' },
  heimish: { label: 'ביתי ומחבר', tag: 'STYLE_HEIMISH', icon: <Heart className="w-5 h-5" />, color: 'bg-orange-400' },
};
...
const StepSummary = ({ data, updateData, onComplete, onPrev }: StepSummaryProps) => {
  ...
  const handleSubmit = () => {
    if (!data.contactName || !data.contactPhone) {
      toast.error('נא למלא שם וטלפון ליצירת קשר');
      return;
    }
    
    toast.success('הבריף נשלח בהצלחה! ניצור קשר בהקדם');
    onComplete();
  };
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      <div className="text-center space-y-3">
        ...
        <h1 className="text-3xl font-bold text-foreground">בריף הקמפיין שלך</h1>
        <p className="text-lg text-muted-foreground">
          סיכום הבחירות שלך - הכל מוכן לייצור!
        </p>
      </div>
      ...
      <div className="bg-gradient-to-br from-primary/5 via-card to-primary/5 rounded-2xl border-2 border-primary/20 overflow-hidden">
        ...
        <div className="bg-gradient-primary p-6 text-primary-foreground">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm font-medium opacity-90">בריף קמפיין</span>
          </div>
        ...
        <div className="p-6 space-y-6">
          ...
          {vibeInfo && (
            <div className="flex items-center gap-4">
              <div className={`w-14 h-14 rounded-xl ${vibeInfo.color} flex items-center justify-center text-white`}>
                {vibeInfo.icon}
              </div>
              <div>
      ...
      <div className="bg-card rounded-xl border border-border p-6 space-y-4">
        <h3 className="text-lg font-semibold text-foreground">פרטי יצירת קשר</h3>
